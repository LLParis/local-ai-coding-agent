...
|---|---|---|
| PC inference lifecycle | Bounded Qwen3.8 Q6, native 262K Qwen, and Ollama are mutually exclusive, loopback-only, exactly owned Limited on-demand Scheduled Tasks with rollback and no UAC. Normal idle state is `Off`; Native retains Q6/262K while adaptive fit targets an 8 GiB GPU margin for display headroom when active | `bin/doctor.ps1`; lifecycle evidence under `runs/` |
...
| Current production harness | Everyday `bin/coding.cmd` repository-objective command: DeepSeek Harness drives Qwen3.8-27B Q6 Native 262K through an inspect/edit/test loop in an isolated stage, runs the real project command, bounds repairs to two materially new failures, takes one advisory Devstral review, unloads inference afterward, and applies a verified result by default | `bin/coding.cmd`; `src/agent_continuity/production_worker.py`; `runs/live-foundation-completion.json` |
